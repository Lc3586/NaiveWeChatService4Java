 ...
 */
@RestController
@RequestMapping(path = "/wechat/user-info",
...
public class UserInfoController
     ...
     * @return 列表数据
     */
    @PreAuthorize("@aph.isPass('wechat:user-info:list')")
    @PostMapping("/list")
    @ApiOperation("获取列表数据")
    @OpenApiModels(defaultGenericTypes = {@OpenApiGenericType(arguments = {@OpenApiGenericTypeArgument(UI_List.class)})},
    ...
    public Object list(
        ...
        return JsonExtension.toOpenApiJson(ApiDataSchemaExtension.buildResult(dataSearch.getSchema(),
                                                                              userInfoService.list(dataSearch),
     ...
     * @return 详情数据
     */
    @PreAuthorize("@aph.isPass('wechat:user-info:detail')")
    @GetMapping(value = "/detail/{id}",
                consumes = "*/*")
    @ApiOperation("获取详情数据")
    @OpenApiModel(value = UI_Detail.class,
                  description = "详情数据")
    public Object detail(
            ...
            @PathVariable("id")
                    String id) {
        return JsonExtension.toOpenApiJson(ApiResultData.success(userInfoService.detail(id)),
                                           UI_Detail.class);
    }
     ...
     * @param data 数据
     */
    @PreAuthorize("@aph.isPass('wechat:user-info:create')")
    @PostMapping("/create")
    @ApiOperation("新增")
     ...
     * @return 编辑数据
     */
    @PreAuthorize("@aph.isPass('wechat:user-info:edit')")
    @GetMapping(value = "/edit/{id}",
                consumes = "*/*")
    @ApiOperation("获取编辑数据")
    @OpenApiModel(value = UI_Edit.class,
                  description = "编辑数据")
    public Object edit(
            @Parameter(name = "id",
                       description = "主键")
            @PathVariable("id")
                    String id) {
        return JsonExtension.toOpenApiJson(ApiResultData.success(userInfoService.edit(id)),
                                           UI_Edit.class);
    }
     ...
     * @param ids 主键集合
     */
    @PreAuthorize("@aph.isPass('wechat:user-info:delete')")
    @PostMapping("/delete")
    @ApiOperation("删除")
